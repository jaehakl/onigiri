...
async def api_create_words_personal(request: Request, data_json: str = Form(...), file_meta_json: str = Form("[]"), 
    ...
    return await auth_service_async(request, ["admin", "user"], db, create_words_personal, data_json, file_meta_json, files)
...
async def api_create_user_text(request: Request, user_text_data: TextData, db: Session = Depends(get_db)):
    return await auth_service_async(request, ["admin", "user"], db, create_user_text, user_text_data)

@app.get("/user_text/get/{user_text_id}")
async def api_get_user_text(request: Request, user_text_id: int, db: Session = Depends(get_db)):
    return await auth_service_async(request, ["admin", "user"], db, get_user_text, user_text_id)
...
async def api_get_user_text_list(request: Request, limit: int = None, offset: int = None, db: Session = Depends(get_db)):
    return await auth_service_async(request, ["admin", "user"], db, get_user_text_list, limit, offset)

@app.post("/user_text/update")
async def api_update_user_text(request: Request, user_text_data: TextData, db: Session = Depends(get_db)):
    return await auth_service_async(request, ["admin", "user"], db, update_user_text, user_text_data)

@app.get("/user_text/delete/{user_text_id}")
async def api_delete_user_text(request: Request, user_text_id: int, db: Session = Depends(get_db)):
    return await auth_service_async(request, ["admin", "user"], db, delete_user_text, user_text_id)
...
async def api_delete_user(request: Request, id: str, db: Session = Depends(get_db)):
    return auth_service(request, ["admin"], db, UserService.delete_user, id)
