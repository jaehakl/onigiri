...
async def api_create_words_batch(request: Request, words_data: List[WordData], db: Session = Depends(get_db), user: UserData = Depends(check_user)):
    return auth_service(request, ["admin"], db, user, create_words_batch, words_data)

@app.post("/words/update/batch")
async def api_update_words(request: Request, words_data: List[WordData], db: Session = Depends(get_db), user: UserData = Depends(check_user)):    
    return auth_service(request, ["admin"], db, user, update_words_batch, words_data)

@app.post("/words/delete/batch")
async def api_delete_words(request: Request, word_ids: List[int], db: Session = Depends(get_db), user: UserData = Depends(check_user)):
    return auth_service(request, ["admin"], db, user, delete_words_batch, word_ids)

@app.post("/words/all")
async def api_get_words(request: Request, data: Dict[str, Any], db: Session = Depends(get_db), user: UserData = Depends(check_user)):
    limit = data.get("limit")
    offset = data.get("offset")
    return auth_service(request, ["*"], db, user, get_all_words, limit, offset)

@app.get("/words/search/{search_term}")
async def api_search_word(request: Request, search_term: str, db: Session = Depends(get_db), user: UserData = Depends(check_user)):
...
async def api_create_words_personal(request: Request, data_json: str = Form(...), file_meta_json: str = Form("[]"), 
                                    files: List[UploadFile] = File(default=[]), db: Session = Depends(get_db), user: UserData = Depends(check_user)):
...
async def api_get_random_words_to_learn(request: Request, limit: int, db: Session = Depends(get_db), user: UserData = Depends(check_user)):
    return auth_service(request, ["admin", "user"], db, user, get_random_words_to_learn, limit)


@app.post("/words/filter")
async def api_filter_words(request: Request, word_filter_data: WordFilterData, db: Session = Depends(get_db), user: UserData = Depends(check_user)):
...
async def api_create_examples(request: Request, examples_data: List[ExampleData], db: Session = Depends(get_db), user: UserData = Depends(check_user)):
    return auth_service(request, ["admin"], db, user, create_examples_batch, examples_data)

@app.post("/examples/update/batch")
async def api_update_examples(request: Request, examples_data: List[ExampleData], db: Session = Depends(get_db), user: UserData = Depends(check_user)):
    return auth_service(request, ["admin"], db, user, update_examples_batch, examples_data)

@app.post("/examples/delete/batch")
async def api_delete_examples(request: Request, example_ids: List[int], db: Session = Depends(get_db), user: UserData = Depends(check_user)):
    return auth_service(request, ["admin"], db, user, delete_examples_batch, example_ids)

@app.post("/examples/filter")
async def api_filter_examples(request: Request, example_filter_data: ExampleFilterData, db: Session = Depends(get_db), user: UserData = Depends(check_user)):
...
async def api_get_examples_for_user(request: Request, db: Session = Depends(get_db), user: UserData = Depends(check_user)):
...
async def api_create_user_text(request: Request, user_text_data: TextData, db: Session = Depends(get_db), user: UserData = Depends(check_user)):
...
async def api_get_user_text(request: Request, user_text_id: int, db: Session = Depends(get_db), user: UserData = Depends(check_user)):
    return await auth_service_async(request, ["admin", "user"], db, user, get_user_text, user_text_id)

@app.get("/user_text/all")
async def api_get_user_text_list(request: Request, limit: int = None, offset: int = None, db: Session = Depends(get_db), user: UserData = Depends(check_user)):
    return await auth_service_async(request, ["admin", "user"], db, user, get_user_text_list, limit, offset)

@app.post("/user_text/update")
async def api_update_user_text(request: Request, user_text_data: TextData, db: Session = Depends(get_db), user: UserData = Depends(check_user)):
...
async def api_delete_user_text(request: Request, user_text_id: int, db: Session = Depends(get_db), user: UserData = Depends(check_user)):
...
async def api_get_user_list(request: Request, limit: int = None, offset: int = None, db: Session = Depends(get_db), user: UserData = Depends(check_user)):
    return auth_service(request, ["admin"], db, user, UserService.get_users, limit, offset)

@app.get("/user_admin/delete/{id}")
async def api_delete_user(request: Request, id: str, db: Session = Depends(get_db), user: UserData = Depends(check_user)):
    return auth_service(request, ["admin"], db, user, UserService.delete_user, id)

@app.get("/user_data/summary/admin/{user_id}")
async def api_get_user_summary_admin(request: Request, user_id: str, db: Session = Depends(get_db), user: UserData = Depends(check_user)):
    return auth_service(request, ["admin"], db, user, UserService.get_user_summary, user_id)

@app.get("/user_data/all/admin/{user_id}")
async def api_get_user_all_data_admin(request: Request, user_id: str, db: Session = Depends(get_db), user: UserData = Depends(check_user)):
    return auth_service(request, ["admin"], db, user, UserService.get_user_with_all_data, user_id)

@app.get("/user_data/summary/user")
async def api_get_user_summary_user(request: Request, db: Session = Depends(get_db), user: UserData = Depends(check_user)):
    return auth_service(request, ["admin", "user"], db, user, UserService.get_user_summary, "me")

@app.get("/user_data/all/user")
async def api_get_user_all_data_user(request: Request, db: Session = Depends(get_db), user: UserData = Depends(check_user)):
...
async def api_analyze_text(request: Request, text_data: TextData, db: Session = Depends(get_db), user: UserData = Depends(check_user)):
